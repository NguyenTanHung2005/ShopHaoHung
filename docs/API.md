...
**Status**: Product CRUD is backed by shared `localStorage` helpers on the client. The product API routes remain for compatibility, but the UI reads and writes products through `lib/product-storage.ts`.
...
Product create, update, and delete actions go through the shared product storage helper.

- Storage key: `shophaohung.products`
- Seed data: `lib/badminton-products.json`
- Change propagation: custom storage event keeps dashboard and shop views in sync
...
> Compatibility note: the response shape is kept for reference, but the current UI source of truth is localStorage.
...
> Compatibility note: product detail screens resolve data from localStorage so newly created items appear immediately.
