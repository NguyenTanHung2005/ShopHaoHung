...
import { getStoredProduct, getStoredProducts } from '@/lib/product-storage';
...
export default function ClientProductDetail({ id }: { id: string | string[] }) {
  ...
  const product =
    getStoredProduct(idStr) ??
    getStoredProduct(decodeURIComponent(idStr)) ??
    getStoredProduct(String(Number(idStr))) ??
    null;
